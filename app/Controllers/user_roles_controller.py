...
class UserRolesController:
    ...
    async def list_user_roles(self, user_id: UUID, db: AsyncSession = Depends(get_db)) -> list[dict]:
    ...
    async def unassign_role(self, user_id: str, role_id: UUID, db: AsyncSession = Depends(get_db)) -> dict:
        await require_roles(["admin"])()
        svc = RoleService(db)
